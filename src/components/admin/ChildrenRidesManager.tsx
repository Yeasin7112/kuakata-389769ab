...
const ChildrenRidesManager: React.FC = () => {
  ...
  const [formData, setFormData] = useState({
    name_bn: '', name_en: '', description_bn: '', description_en: '', price_bn: '', price_en: '', location_bn: '', location_en: '', timing_bn: '', timing_en: '', image_url: '', is_active: true,
  });

  const { data: items, isLoading } = useQuery({
    queryKey: ['admin-children-rides'],
    queryFn: async () => {
      const { data, error } = await supabase.from('children_rides').select('*').order('created_at', { ascending: false });
  ...
  const createMutation = useMutation({
    mutationFn: async (data: any) => { const { error } = await supabase.from('children_rides').insert([data]); if (error) throw error; },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['admin-children-rides'] }); toast({ title: language === 'bn' ? 'সফলভাবে যোগ করা হয়েছে' : 'Added successfully' }); resetForm(); },
    onError: () => toast({ title: language === 'bn' ? 'ত্রুটি হয়েছে' : 'Error occurred', variant: 'destructive' })
  });

  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: any }) => { const { error } = await supabase.from('children_rides').update(data).eq('id', id); if (error) throw error; },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['admin-children-rides'] }); toast({ title: language === 'bn' ? 'সফলভাবে আপডেট হয়েছে' : 'Updated successfully' }); resetForm(); },
    onError: () => toast({ title: language === 'bn' ? 'ত্রুটি হয়েছে' : 'Error occurred', variant: 'destructive' })
  });

  const deleteMutation = useMutation({
    mutationFn: async (id: string) => { const { error } = await supabase.from('children_rides').delete().eq('id', id); if (error) throw error; },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['admin-children-rides'] }); toast({ title: language === 'bn' ? 'সফলভাবে মুছে ফেলা হয়েছে' : 'Deleted successfully' }); },
    onError: () => toast({ title: language === 'bn' ? 'ত্রুটি হয়েছে' : 'Error occurred', variant: 'destructive' })
  });

  const resetForm = () => { setFormData({ name_bn: '', name_en: '', description_bn: '', description_en: '', price_bn: '', price_en: '', location_bn: '', location_en: '', timing_bn: '', timing_en: '', image_url: '', is_active: true }); setEditingItem(null); setIsDialogOpen(false); };

  const handleEdit = (item: any) => {
    setEditingItem(item);
    setFormData({ name_bn: item.name_bn, name_en: item.name_en, description_bn: item.description_bn || '', description_en: item.description_en || '', price_bn: item.price_bn || '', price_en: item.price_en || '', location_bn: item.location_bn || '', location_en: item.location_en || '', timing_bn: item.timing_bn || '', timing_en: item.timing_en || '', image_url: item.image_url || '', is_active: item.is_active ?? true });
    setIsDialogOpen(true);
  };

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const data = { ...formData, description_bn: formData.description_bn || null, description_en: formData.description_en || null, price_bn: formData.price_bn || null, price_en: formData.price_en || null, location_bn: formData.location_bn || null, location_en: formData.location_en || null, timing_bn: formData.timing_bn || null, timing_en: formData.timing_en || null, image_url: formData.image_url || null };
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-bold font-bangla">{language === 'bn' ? 'শিশুদের রাইড ম্যানেজমেন্ট' : 'Children Rides Management'}</h2>
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div><Label>মূল্য (বাংলা)</Label><Input value={formData.price_bn} onChange={(e) => setFormData({...formData, price_bn: e.target.value})} /></div>
                <div><Label>Price (English)</Label><Input value={formData.price_en} onChange={(e) => setFormData({...formData, price_en: e.target.value})} /></div>
              </div>
              <div className="grid grid-cols-2 gap-4">
              ...
              <div><Label>ছবি</Label><ImageUpload currentImage={formData.image_url} onImageUploaded={(url) => setFormData({...formData, image_url: url})} folder="rides" /></div>
...
export default ChildrenRidesManager;
