...
const RestaurantsManager: React.FC = () => {
  ...
  const resetForm = () => {
    setFormData({ name_bn: '', name_en: '', description_bn: '', description_en: '', address_bn: '', address_en: '', phone: '', cuisine_type: '', price_range: '', rating: 0, image_url: '', is_active: true });
    setEditing(null);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editing) {
        await supabase.from('restaurants').update(formData).eq('id', editing.id);
      } else {
        await supabase.from('restaurants').insert([formData]);
      }
    ...
    <div className="space-y-6">
          ...
          <div key={item.id} className="card-elevated p-4 flex justify-between items-center">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">{item.cuisine_type} • {item.price_range}</p>
              </div>
            </div>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-h-[90vh] overflow-y-auto">
          <DialogHeader><DialogTitle>{editing ? 'Edit' : 'Add'} Restaurant</DialogTitle></DialogHeader>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div><Label>Address (BN)</Label><Input value={formData.address_bn} onChange={(e) => setFormData({...formData, address_bn: e.target.value})} /></div>
              <div><Label>Address (EN)</Label><Input value={formData.address_en} onChange={(e) => setFormData({...formData, address_en: e.target.value})} /></div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div><Label>Phone</Label><Input value={formData.phone} onChange={(e) => setFormData({...formData, phone: e.target.value})} /></div>
              <div><Label>Cuisine Type</Label><Input value={formData.cuisine_type} onChange={(e) => setFormData({...formData, cuisine_type: e.target.value})} /></div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div><Label>Price Range</Label><Input value={formData.price_range} placeholder="$$" onChange={(e) => setFormData({...formData, price_range: e.target.value})} /></div>
              <div><Label>Rating</Label><Input type="number" min="0" max="5" step="0.1" value={formData.rating} onChange={(e) => setFormData({...formData, rating: parseFloat(e.target.value)})} /></div>
            </div>
