...
const BeachChairsManager: React.FC = () => {
  const { language } = useLanguage();
  const { toast } = useToast();
  const queryClient = useQueryClient();
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<BeachChair | null>(null);
  ...
  const { data: items, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return data as BeachChair[];
    }
  });

  const createMutation = useMutation({
    mutationFn: async (data: any) => {
      const { error } = await supabase.from('beach_chairs').insert([data]);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-beach-chairs'] });
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: any }) => {
      const { error } = await supabase.from('beach_chairs').update(data).eq('id', id);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-beach-chairs'] });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from('beach_chairs').delete().eq('id', id);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-beach-chairs'] });
  ...
  const resetForm = () => {
    setFormData({
      name_bn: '', name_en: '', price_bn: '', price_en: '', timing_bn: '', timing_en: '',
      phone: '', features_bn: '', features_en: '', location_bn: '', location_en: '', image_url: '', is_active: true,
    });
    setEditingItem(null);
    setIsDialogOpen(false);
  };

  const handleEdit = (item: BeachChair) => {
    setEditingItem(item);
    setFormData({
      ...
      timing_bn: item.timing_bn || '',
      timing_en: item.timing_en || '',
      phone: item.phone || '',
      features_bn: item.features_bn?.join(', ') || '',
      features_en: item.features_en?.join(', ') || '',
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const data = {
      ...
      price_bn: formData.price_bn || null,
      price_en: formData.price_en || null,
      timing_bn: formData.timing_bn || null,
      timing_en: formData.timing_en || null,
      phone: formData.phone || null,
      features_bn: formData.features_bn ? formData.features_bn.split(',').map(f => f.trim()) : null,
      features_en: formData.features_en ? formData.features_en.split(',').map(f => f.trim()) : null,
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-bold font-bangla">
          {language === 'bn' ? 'কিটকট চেয়ার ম্যানেজমেন্ট' : 'Beach Chairs Management'}
        </h2>
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogTrigger asChild>
            <Button onClick={() => resetForm()}><Plus className="w-4 h-4 mr-2" /> {language === 'bn' ? 'নতুন যোগ করুন' : 'Add New'}</Button>
          </DialogTrigger>
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle className="font-bangla">
                {editingItem ? (language === 'bn' ? 'সম্পাদনা করুন' : 'Edit') : (language === 'bn' ? 'নতুন যোগ করুন' : 'Add New')}
              </DialogTitle>
            </DialogHeader>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div><Label>সুবিধাসমূহ (বাংলা, কমা দিয়ে)</Label><Input value={formData.features_bn} onChange={(e) => setFormData({...formData, features_bn: e.target.value})} placeholder="ছাতা, চেয়ার, টেবিল" /></div>
                <div><Label>Features (English, comma separated)</Label><Input value={formData.features_en} onChange={(e) => setFormData({...formData, features_en: e.target.value})} placeholder="Umbrella, Chair, Table" /></div>
              </div>
              <div>
                <Label>ছবি</Label>
          ...
          <div key={item.id} className="card-elevated p-4 flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">{item.location_bn || item.location_en}</p>
                <span className={`text-xs px-2 py-0.5 rounded-full ${item.is_active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                  {item.is_active ? (language === 'bn' ? 'সক্রিয়' : 'Active') : (language === 'bn' ? 'নিষ্ক্রিয়' : 'Inactive')}
                </span>
              </div>
            </div>
...
export default BeachChairsManager;
