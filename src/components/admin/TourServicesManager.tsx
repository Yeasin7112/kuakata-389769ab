...
const TourServicesManager: React.FC = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    service_type: 'speed_boat', price_bn: '', price_en: '', phone: '', image_url: '', is_active: true,
  });

  const serviceTypes = [
    { value: 'speed_boat', labelBn: 'স্পিড বোট', labelEn: 'Speed Boat' },
    { value: 'beach_bike', labelBn: 'বিচ বাইক', labelEn: 'Beach Bike' },
    { value: 'sundarbans', labelBn: 'সুন্দরবন ভ্রমণ', labelEn: 'Sundarbans Tour' },
    { value: 'boat', labelBn: 'নৌকা', labelEn: 'Boat' },
    { value: 'houseboat', labelBn: 'হাউজবোট', labelEn: 'Houseboat' },
  ];

  const { data: items, isLoading } = useQuery({
    queryKey: ['admin-tour-services'],
    queryFn: async () => {
      const { data, error } = await supabase.from('tour_services').select('*').order('created_at', { ascending: false });
  ...
  const createMutation = useMutation({
    mutationFn: async (data: any) => {
      const { error } = await supabase.from('tour_services').insert([data]);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-tour-services'] });
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: any }) => {
      const { error } = await supabase.from('tour_services').update(data).eq('id', id);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-tour-services'] });
  ...
  const resetForm = () => {
    setFormData({ name_bn: '', name_en: '', description_bn: '', description_en: '', service_type: 'speed_boat', price_bn: '', price_en: '', phone: '', image_url: '', is_active: true });
    setEditingItem(null);
  ...
  const handleEdit = (item: any) => {
    setEditingItem(item);
    setFormData({
      name_bn: item.name_bn, name_en: item.name_en, description_bn: item.description_bn || '', description_en: item.description_en || '',
      service_type: item.service_type, price_bn: item.price_bn || '', price_en: item.price_en || '', phone: item.phone || '', image_url: item.image_url || '', is_active: item.is_active ?? true,
    });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const data = { ...formData, description_bn: formData.description_bn || null, description_en: formData.description_en || null, price_bn: formData.price_bn || null, price_en: formData.price_en || null, phone: formData.phone || null, image_url: formData.image_url || null };
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-bold font-bangla">{language === 'bn' ? 'ট্যুর সার্ভিস ম্যানেজমেন্ট' : 'Tour Services Management'}</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <Select value={formData.service_type} onValueChange={(value) => setFormData({...formData, service_type: value})}>
                  ...
                  <SelectContent>{serviceTypes.map(t => <SelectItem key={t.value} value={t.value}>{language === 'bn' ? t.labelBn : t.labelEn}</SelectItem>)}</SelectContent>
                </Select>
              </div>
              <div className="grid grid-cols-2 gap-4">
              ...
              <div><Label>ছবি</Label><ImageUpload value={formData.image_url} onChange={(url) => setFormData({...formData, image_url: url})} folder="tour-services" /></div>
        ...
        {items?.map((item) => (
          <div key={item.id} className="card-elevated p-4 flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <span className="text-xs bg-primary/10 text-primary px-2 py-0.5 rounded-full">{serviceTypes.find(t => t.value === item.service_type)?.[language === 'bn' ? 'labelBn' : 'labelEn']}</span>
...
export default TourServicesManager;
